...
def build_menu_tree_html(menu_list):
    tpl1 = """
       <div class='rbac-menu-item'>
            <a class='menu-item'href="#"><i class="fa fa-desktop"aria-hidden="true"></i><span>{0}:</span></a>
...
@register.simple_tag
def rbac_css():
    file_path = os.path.join(settings.BASE_DIR, 'rbac','theme', settings.RBAC_THEME, 'rbac.css')
...
@register.simple_tag
def rbac_js():
    file_path = os.path.join(settings.BASE_DIR, 'rbac','theme', settings.RBAC_THEME, 'rbac.js')
    if os.path.exists(file_path):
        return mark_safe(open(file_path, 'r', encoding='utf-8').read())
    else:
