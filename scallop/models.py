...
class Record(models.Model):
    ...
    申请记录表,该记录表自动记录
    """
    apply_for = models.ForeignKey(verbose_name='申请表', to="BaseApply")
...
class BaseApply(models.Model):
    ...
    所有申请表所要继承的父类
    """
    apply_id = models.CharField(verbose_name="订单号", max_length=16, null=True,
                                blank=True)  # 部门id+今天日期++流水号001自动生成12位不重复数字的字符串
    depart = models.ForeignKey(verbose_name="所属部门", to=Department)  # 申请部门暂时留这样的多对多。自动获取,
    user = models.ForeignKey(verbose_name="申请人", to=User)  # 申请人因为还没有与rbac挂钩暂时先以单表的方式存在。自动获取,显示为用户的工号
